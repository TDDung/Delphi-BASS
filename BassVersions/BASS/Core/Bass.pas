...
{$IFDEF MACOS}
  {$IFDEF IOS} // Needed frameworks: AudioToolbox, SystemConfiguration, CFNetwork, Accelerate, CoreMIDI (if using BASSMIDI)
    BassDLL = 'Bass\Core\Libs\iOS\libbass.a'; // Full path for static linking
  {$ELSE}
...
function BASS_IsAvailable: Boolean;
...
//{$REGION '=== Work-around for Linux to support loading of BASS_OPUS ==='}
//{$IFDEF POSIX}
//uses
//  Posix.Dlfcn;
//{$ENDIF}
...
//{$REGION '=== Work-around for Linux to support loading of BASS_OPUS ==='}
//  {$IFDEF POSIX}
//  function LinuxLoadLibrary(ModuleName: PChar): HMODULE;
//  var
//    M: TMarshaller;
//  begin
//    Result := HMODULE(dlopen(M.AsAnsi(ModuleName, CP_UTF8).ToPointer, RTLD_LAZY or RTLD_GLOBAL));
//  end;
//  {$ENDIF}
//{$ENDREGION}
...
begin
//  {$IFDEF POSIX}
//    FBassDLL := LinuxLoadLibrary(PChar(BassDLL));
//  {$ELSE}
...
//  {$ENDIF}
  ...
  BASS_SetConfig := GetProcAddress(FBassDLL, PChar('BASS_SetConfig'));
  BASS_GetConfig := GetProcAddress(FBassDLL, PChar('BASS_GetConfig'));
  BASS_SetConfigPtr := GetProcAddress(FBassDLL, PChar('BASS_SetConfigPtr'));
  BASS_GetConfigPtr := GetProcAddress(FBassDLL, PChar('BASS_GetConfigPtr'));
  BASS_GetVersion := GetProcAddress(FBassDLL, PChar('BASS_GetVersion'));
  BASS_ErrorGetCode := GetProcAddress(FBassDLL, PChar('BASS_ErrorGetCode'));
  BASS_GetDeviceInfo := GetProcAddress(FBassDLL, PChar('BASS_GetDeviceInfo'));
  BASS_Init := GetProcAddress(FBassDLL, PChar('BASS_Init'));
  BASS_SetDevice := GetProcAddress(FBassDLL, PChar('BASS_SetDevice'));
  BASS_GetDevice := GetProcAddress(FBassDLL, PChar('BASS_GetDevice'));
  BASS_Free := GetProcAddress(FBassDLL, PChar('BASS_Free'));
{$IFDEF MSWINDOWS}
  BASS_GetDSoundObject := GetProcAddress(FBassDLL, PChar('BASS_GetDSoundObject'));
{$ENDIF}
  BASS_GetInfo := GetProcAddress(FBassDLL, PChar('BASS_GetInfo'));
  BASS_Update := GetProcAddress(FBassDLL, PChar('BASS_Update'));
  BASS_GetCPU := GetProcAddress(FBassDLL, PChar('BASS_GetCPU'));
  BASS_Start := GetProcAddress(FBassDLL, PChar('BASS_Start'));
  BASS_Stop := GetProcAddress(FBassDLL, PChar('BASS_Stop'));
  BASS_Pause := GetProcAddress(FBassDLL, PChar('BASS_Pause'));
  BASS_SetVolume := GetProcAddress(FBassDLL, PChar('BASS_SetVolume'));
  BASS_GetVolume := GetProcAddress(FBassDLL, PChar('BASS_GetVolume'));
  BASS_PluginLoad := GetProcAddress(FBassDLL, PChar('BASS_PluginLoad'));
  BASS_PluginFree := GetProcAddress(FBassDLL, PChar('BASS_PluginFree'));
  BASS_PluginGetInfo := GetProcAddress(FBassDLL, PChar('BASS_PluginGetInfo'));
  BASS_Set3DFactors := GetProcAddress(FBassDLL, PChar('BASS_Set3DFactors'));
  BASS_Get3DFactors := GetProcAddress(FBassDLL, PChar('BASS_Get3DFactors'));
  BASS_Set3DPosition := GetProcAddress(FBassDLL, PChar('BASS_Set3DPosition'));
  BASS_Get3DPosition := GetProcAddress(FBassDLL, PChar('BASS_Get3DPosition'));
  BASS_Apply3D := GetProcAddress(FBassDLL, PChar('BASS_Apply3D'));
{$IFDEF MSWINDOWS}
  BASS_SetEAXParameters := GetProcAddress(FBassDLL, PChar('BASS_SetEAXParameters'));
  BASS_GetEAXParameters := GetProcAddress(FBassDLL, PChar('BASS_GetEAXParameters'));
{$ENDIF}
  BASS_MusicLoad := GetProcAddress(FBassDLL, PChar('BASS_MusicLoad'));
  BASS_MusicFree := GetProcAddress(FBassDLL, PChar('BASS_MusicFree'));
  BASS_SampleLoad := GetProcAddress(FBassDLL, PChar('BASS_SampleLoad'));
  BASS_SampleCreate := GetProcAddress(FBassDLL, PChar('BASS_SampleCreate'));
  BASS_SampleFree := GetProcAddress(FBassDLL, PChar('BASS_SampleFree'));
  BASS_SampleSetData := GetProcAddress(FBassDLL, PChar('BASS_SampleSetData'));
  BASS_SampleGetData := GetProcAddress(FBassDLL, PChar('BASS_SampleGetData'));
  BASS_SampleGetInfo := GetProcAddress(FBassDLL, PChar('BASS_SampleGetInfo'));
  BASS_SampleSetInfo := GetProcAddress(FBassDLL, PChar('BASS_SampleSetInfo'));
  BASS_SampleGetChannel := GetProcAddress(FBassDLL, PChar('BASS_SampleGetChannel'));
  BASS_SampleGetChannels := GetProcAddress(FBassDLL, PChar('BASS_SampleGetChannels'));
  BASS_SampleStop := GetProcAddress(FBassDLL, PChar('BASS_SampleStop'));
  BASS_StreamCreate := GetProcAddress(FBassDLL, PChar('BASS_StreamCreate'));
  BASS_StreamCreateFile := GetProcAddress(FBassDLL, PChar('BASS_StreamCreateFile'));
  BASS_StreamCreateURL := GetProcAddress(FBassDLL, PChar('BASS_StreamCreateURL'));
  BASS_StreamCreateFileUser := GetProcAddress(FBassDLL, PChar('BASS_StreamCreateFileUser'));
  BASS_StreamFree := GetProcAddress(FBassDLL, PChar('BASS_StreamFree'));
  BASS_StreamGetFilePosition := GetProcAddress(FBassDLL, PChar('BASS_StreamGetFilePosition'));
  BASS_StreamPutData := GetProcAddress(FBassDLL, PChar('BASS_StreamPutData'));
  BASS_StreamPutFileData := GetProcAddress(FBassDLL, PChar('BASS_StreamPutFileData'));
  BASS_RecordGetDeviceInfo := GetProcAddress(FBassDLL, PChar('BASS_RecordGetDeviceInfo'));
  BASS_RecordInit := GetProcAddress(FBassDLL, PChar('BASS_RecordInit'));
  BASS_RecordSetDevice := GetProcAddress(FBassDLL, PChar('BASS_RecordSetDevice'));
  BASS_RecordGetDevice := GetProcAddress(FBassDLL, PChar('BASS_RecordGetDevice'));
  BASS_RecordFree := GetProcAddress(FBassDLL, PChar('BASS_RecordFree'));
  BASS_RecordGetInfo := GetProcAddress(FBassDLL, PChar('BASS_RecordGetInfo'));
  BASS_RecordGetInputName := GetProcAddress(FBassDLL, PChar('BASS_RecordGetInputName'));
  BASS_RecordSetInput := GetProcAddress(FBassDLL, PChar('BASS_RecordSetInput'));
  BASS_RecordGetInput := GetProcAddress(FBassDLL, PChar('BASS_RecordGetInput'));
  BASS_RecordStart := GetProcAddress(FBassDLL, PChar('BASS_RecordStart'));
  BASS_ChannelBytes2Seconds := GetProcAddress(FBassDLL, PChar('BASS_ChannelBytes2Seconds'));
  BASS_ChannelSeconds2Bytes := GetProcAddress(FBassDLL, PChar('BASS_ChannelSeconds2Bytes'));
  BASS_ChannelGetDevice := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetDevice'));
  BASS_ChannelSetDevice := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetDevice'));
  BASS_ChannelIsActive := GetProcAddress(FBassDLL, PChar('BASS_ChannelIsActive'));
  BASS_ChannelGetInfo := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetInfo'));
  BASS_ChannelGetTags := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetTags'));
  BASS_ChannelFlags := GetProcAddress(FBassDLL, PChar('BASS_ChannelFlags'));
  BASS_ChannelUpdate := GetProcAddress(FBassDLL, PChar('BASS_ChannelUpdate'));
  BASS_ChannelLock := GetProcAddress(FBassDLL, PChar('BASS_ChannelLock'));
  BASS_ChannelPlay := GetProcAddress(FBassDLL, PChar('BASS_ChannelPlay'));
  BASS_ChannelStop := GetProcAddress(FBassDLL, PChar('BASS_ChannelStop'));
  BASS_ChannelPause := GetProcAddress(FBassDLL, PChar('BASS_ChannelPause'));
  BASS_ChannelSetAttribute := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetAttribute'));
  BASS_ChannelGetAttribute := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetAttribute'));
  BASS_ChannelSlideAttribute := GetProcAddress(FBassDLL, PChar('BASS_ChannelSlideAttribute'));
  BASS_ChannelIsSliding := GetProcAddress(FBassDLL, PChar('BASS_ChannelIsSliding'));
  BASS_ChannelSet3DAttributes := GetProcAddress(FBassDLL, PChar('BASS_ChannelSet3DAttributes'));
  BASS_ChannelGet3DAttributes := GetProcAddress(FBassDLL, PChar('BASS_ChannelGet3DAttributes'));
  BASS_ChannelSet3DPosition := GetProcAddress(FBassDLL, PChar('BASS_ChannelSet3DPosition'));
  BASS_ChannelGet3DPosition := GetProcAddress(FBassDLL, PChar('BASS_ChannelGet3DPosition'));
  BASS_ChannelGetLength := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetLength'));
  BASS_ChannelSetPosition := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetPosition'));
  BASS_ChannelGetPosition := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetPosition'));
  BASS_ChannelGetLevel := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetLevel'));
  BASS_ChannelGetData := GetProcAddress(FBassDLL, PChar('BASS_ChannelGetData'));
  BASS_ChannelSetSync := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetSync'));
  BASS_ChannelRemoveSync := GetProcAddress(FBassDLL, PChar('BASS_ChannelRemoveSync'));
  BASS_ChannelSetDSP := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetDSP'));
  BASS_ChannelSetLink := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetLink'));
  BASS_ChannelRemoveLink := GetProcAddress(FBassDLL, PChar('BASS_ChannelRemoveLink'));
  BASS_ChannelSetFX := GetProcAddress(FBassDLL, PChar('BASS_ChannelSetFX'));
  BASS_ChannelRemoveFX := GetProcAddress(FBassDLL, PChar('BASS_ChannelRemoveFX'));
  BASS_FXSetParameters := GetProcAddress(FBassDLL, PChar('BASS_FXSetParameters'));
  BASS_FXGetParameters := GetProcAddress(FBassDLL, PChar('BASS_FXGetParameters'));
  BASS_FXReset := GetProcAddress(FBassDLL, PChar('BASS_FXReset'));
end;
